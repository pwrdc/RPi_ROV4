...
def swim_from_file(main_obj,file):
    csv_reader = csv.reader(file, delimiter=';')
    for row in csv_reader:
        values = []
        for vals in row:
            values += int(vals)
        main_obj.comm.set_engine_driver_values(values[0],values[1],
        values[2],values[3].values[4],values[5])
        time.sleep(SWIMMING_REPLY_SLEEP)

if __name__== "__main__":
    main = Main()
    main.comm.start()
    with open('movements_path.csv','r') as path_file:
        swim_from_file(main,path_file)
    main.comm.join()
    #St
